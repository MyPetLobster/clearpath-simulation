...
class Scoreboard:
    ...
    def draw(self, win):
        ...
        pg.draw.rect(win, (30, 30, 30), (WIDTH - 8 * TILE_SIZE, HEIGHT - 5 * TILE_SIZE, 6 * TILE_SIZE, 4 * TILE_SIZE), border_radius=10)


        if self.clearpath_enabled:
            clear_path_text = self.font.render("ClearPath: Enabled", True, (0, 255, 0))
            win.blit(clear_path_text, (WIDTH - 8 * TILE_SIZE + 10, HEIGHT - 4.5 * TILE_SIZE))
        else:
        ...
        win.blit(timer_text, (WIDTH - 8 * TILE_SIZE + 10, HEIGHT - 3.5 * TILE_SIZE))
